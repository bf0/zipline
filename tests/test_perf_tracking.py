...
shares in position"
    ...
    @parameterized.expand([(start_dt, delete_from_start, delete_from_end)
                          for start_dt in
    ...
    def test_tracker(self, start_dt, delete_from_start, delete_from_end):
        trade_count = 100
        sid = 133
        price = 10.1
        ...
        events = [self.event_with_txn(event,
                                      trade_history[0].dt,
                                      trading_environment)
                  for event in events]
    ...
    def event_with_txn(self, event, no_txn_dt, env):
